...
from app import db
from app.models_main import Lead, VehicleInterest, SavedSearch, SearchHistory, LeadBudget, LeadTimeline, Segment
...
@bp.route('/saved/<int:search_id>/edit', methods=['POST'])
@login_required
def edit_saved_search(search_id):
    ...
    # Check ownership
    if search.user_id != current_user.id:
        flash('You do not have permission to edit this search.', 'danger')
        return redirect(url_for('search.saved_searches'))
    
    # Update search details
    search.name = request.form.get('name')
    search.description = request.form.get('description')
    
    # Handle default status
    is_default = request.form.get('is_default') == '1'
    if is_default and not search.is_default:
    ...
    flash('Saved search updated successfully!', 'success')
    return redirect(url_for('search.saved_searches'))
