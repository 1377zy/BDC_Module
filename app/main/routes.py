...
from app.models import Car, VehicleInterest
...
def index():
    ...
    recent_vehicles = Car.query.filter(
        Car.created_at >= week_ago
    ).order_by(Car.created_at.desc()).limit(5).all()
    
    # Get most popular vehicles (based on lead interest)
    popular_makes = db.session.query(
        VehicleInterest.make, 
        db.func.count(VehicleInterest.id).label('interest_count')
    ).group_by(VehicleInterest.make).order_by(
        db.func.count(VehicleInterest.id).desc()
    ).limit(5).all()
